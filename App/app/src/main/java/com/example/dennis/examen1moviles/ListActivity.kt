    ...
    override fun onContextItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            R.id.item_menu_compartir -> {
                ...
                intent.putExtra(Intent.EXTRA_TEXT, "${getString(R.string.name)} ${actor.nombre} ${actor.apellido}\n${getString(R.string.numero_peliculas)} ${actor.numeroLibros}\n${getString(R.string.fecha_nacimiento)} ${actor.fechaNacimiento}")
            ...
            R.id.item_menu_borrar -> {
                ...
                builder.setMessage(R.string.confirmation)
                        .setPositiveButton(R.string.yes, { dialog, which ->
                            DataBaseActor.deleteActor(actor.id)
                            finish()
                            startActivity(intent)
                        }
                        )
